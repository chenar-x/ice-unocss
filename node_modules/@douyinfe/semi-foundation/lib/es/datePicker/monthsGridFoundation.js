...
export default class MonthsGridFoundation extends BaseFoundation {
  ...
  _initDatePickerFromValue(values) {
    ...
    if (!this._isMultiple()) {
      newSelected.add(format(values[0], strings.FORMAT_FULL_DATE));
    } else {
      _forEachInstanceProperty(values).call(values, date => {
        newSelected.add(format(date, strings.FORMAT_FULL_DATE));
      });
    }
  ...
  handleDateSelected(day, panelType) {
    ...
    const newSelectedDates = _mapInstanceProperty(_context4 = [...newSelected]).call(_context4, _dateStr => compatiableParse(_dateStr, dateFormat, undefined, dateFnsLocale));
  ...
  _mergeDateAndTime(date, time) {
    ...
    const dateStr = format(isValidDate(date) ? date : compatiableParse(date, strings.FORMAT_FULL_DATE, undefined, dateFnsLocale), strings.FORMAT_FULL_DATE);
    const timeStr = format(isValidDate(time) ? time : compatiableParse(time, strings.FORMAT_TIME_PICKER, undefined, dateFnsLocale), strings.FORMAT_TIME_PICKER);
    const timeFormat = this.getValidTimeFormat();
    return compatiableParse(_concatInstanceProperty(_context5 = "".concat(dateStr, " ")).call(_context5, timeStr), timeFormat, undefined, dateFnsLocale);
  }
  ...
  handleRangeSelected(day) {
    ...
    if (rangeStart || rangeEnd) {
      const [startDate, endDate] = [compatiableParse(rangeStart, dateFormat, undefined, dateFnsLocale), compatiableParse(rangeEnd, dateFormat, undefined, dateFnsLocale)];
  ...
  _updateTimeInDateRange(panelType, timeDate) {
    ...
    if (rangeStart && rangeEnd) {
      ...
      let endDate = compatiableParse(rangeEnd, dateFormat, undefined, dateFnsLocale); // console.log('_updateTimeInDateRange()', rangeStart, rangeEnd, startDate, endDate);
